...
namespace SpyStore.Service.Controllers
{
    ...
    public class ShoppingCartRecordController : ControllerBase
    {
        ...
        {
            CartRecordWithProductInfo cartRecordWithProductInfo =
            _repo.GetShoppingCartRecord(recordId);
            return cartRecordWithProductInfo ??
            (ActionResult<CartRecordWithProductInfo>)NotFound();
        }
        ...
        {
            if (record == null || customerId != record.CustomerId
            || !ModelState.IsValid)
            {
                return BadRequest();
            }
