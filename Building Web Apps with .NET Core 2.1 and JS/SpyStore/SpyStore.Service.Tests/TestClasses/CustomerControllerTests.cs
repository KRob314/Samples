...
namespace SpyStore.Service.Tests.TestClasses
{
    ...
    public class CustomerControllerTests : BaseTestClass
    {
        ...
        {
            //Get All Customers: http://localhost:7940/api/customer
        ...
        {
            //Get One customer: http://localhost:7940/api/customer/0
            using (var client = new HttpClient())
            {
                var response =
                await client.GetAsync($"{ServiceAddress}{RootAddress}/1");
                Assert.True(response.IsSuccessStatusCode);
                var jsonResponse = await response.Content.ReadAsStringAsync();
        ...
        {
            //Get One Category: http://localhost:7940/api/customer/1
            using (var client = new HttpClient())
            {
                var response =
                await client.GetAsync($"{ServiceAddress}{RootAddress}/2");
